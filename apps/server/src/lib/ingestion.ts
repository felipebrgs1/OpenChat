...
export async function processIngestion(ingestionId: string): Promise<ProcessResult> {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "ingestão: embeddings falhou, salvando sem embedding",
...
export async function processQueuedIngestions(limit = 5): Promise<string[]> {
  const queued = await db
    ...
    .where(eq(knowledgeIngestions.status, "queued"))
    .limit(limit);
  const ids: string[] = [];
...
export function enqueueIngestionAsync(ingestionId: string): void {
  setTimeout(() => {
    void processIngestion(ingestionId).catch((e) =>
      console.warn("enqueueIngestionAsync failed", e),
    );
  }, 100);
}
